...
class _AdminWebHomeScreenState extends State<AdminWebHomeScreen> {
  ...
  Future<void> _fetchRoute(gmaps.LatLng start, gmaps.LatLng end) async {
    ...
    final String baseUrl = kDebugMode ? devUrl : directionsUrl;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // ✅ FIX: Checking "success" field instead of "status"
        if (data["success"] == true && data["directions"]["routes"].isNotEmpty) {
          List<gmaps.LatLng> newPolylinePoints = [];

          for (var route in data["directions"]["routes"]) {
            for (var leg in route["legs"]) {
              for (var step in leg["steps"]) {
                final endLocation = step["end_location"];
                newPolylinePoints.add(
                  gmaps.LatLng(endLocation["lat"], endLocation["lng"]),
                );
              }
            }
          }
          ...
          print("❌ No route found. API Response: $data");
        }
      } else {
        print("⚠️ Failed to fetch directions: ${response.statusCode} - ${response.reasonPhrase}");
      }
    } catch (error, stacktrace) {
      print("❌ Error fetching directions: $error");
      print(stacktrace); // Debugging
    }
  }
